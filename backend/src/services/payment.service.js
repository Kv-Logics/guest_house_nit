...
exports.getProofHistory = async (bookingId) => {
    ...
    const warnings = await db.query('SELECT w.*, u.full_name as issued_by FROM payment_warnings w LEFT JOIN users u ON w.issued_by_user_id = u.user_id WHERE w.booking_id = $1 ORDER BY w.created_at DESC', [bookingId]);
    return { proofs: proofs.rows, warnings: warnings.rows };
};
...
exports.sendWarning = async (bookingId, adminId, warningLevel, message) => {
    ...
    try {
        ...
        const newState = stateMap[warningLevel];
...
exports.posComplete = async (bookingId, adminId) => {
    const client = await db.getClient();
    try {
        await client.query('BEGIN');
