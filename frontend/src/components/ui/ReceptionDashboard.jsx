        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 animate-fade-in">
            <div className="flex items-center gap-4 mb-8 border-b border-slate-100 pb-6">
                ...
                <div>
                    ...
                    <p className="text-slate-500 font-medium">Manage daily check-ins and active stays</p>
                </div>
            </div>
                            ...
                            {arrivals.map(b => (
                                <tr key={b.booking_id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="p-4 text-right space-x-2">
                                        <button onClick={() => setPreviewId(b.booking_id)} className="inline-flex items-center px-4 py-2 bg-slate-50 text-slate-700 font-bold border border-slate-200 rounded-xl hover:bg-slate-100 transition-colors shadow-sm">
                                            <Eye className="w-4 h-4 mr-1.5" /> Preview
                                        </button>
                                        {b.booking_state === 'CONFIRMED' && (
                                            <button 
                                                onClick={() => checkInMutation.mutate(b.booking_id)} 
                                                disabled={checkInMutation.isPending}
                                                className="px-4 py-2 bg-teal-600 text-white font-bold rounded-xl hover:bg-teal-700 transition-colors disabled:opacity-50"
                                            >
