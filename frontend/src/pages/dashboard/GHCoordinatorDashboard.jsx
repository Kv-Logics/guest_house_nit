...
import SMTPSettingsForm from '../../components/reception/SMTPSettingsForm';
...
import { getFormattedBookingId, getGstRate } from '../../utils/booking';
...
export default function GHCoordinatorDashboard() {
    ...
    const [configSubTab, setConfigSubTab] = useState('billing'); // 'billing' | 'smtp' | 'add_room' | 'tariff'
    ...
    const handleGenerateFinalBill = async () => {
        ...
        try {
            ...
            const calcSubtotal = Math.round(overridePayload.newTotalAmount / (1 + gstRate / 100));
            const calcGst = overridePayload.newTotalAmount - calcSubtotal;
            const billData = {
    ...
    return (
        <div className="space-y-6 w-full pb-12 animate-fade-in px-2">
            {/* Header */}
            ...
            {activeTab === 'config' && (
                <div className="flex flex-col md:flex-row gap-6 w-full items-start mt-4">
                    {/* Left Nav (Vertical Sub-tabs) */}
                    <div className="w-full md:w-64 flex-shrink-0 flex flex-col gap-1.5 border-b md:border-b-0 md:border-r border-slate-200 pb-4 md:pb-0 md:pr-4">
                        <button
                            onClick={() => setConfigSubTab('billing')}
                            className={`w-full text-left px-4 py-3 rounded-xl font-bold transition-all flex items-center gap-3 ${
                                configSubTab === 'billing'
                        ...
                        >
                            ...
                            <span>System Features & Policies</span>
                        </button>
                        <button
                            onClick={() => setConfigSubTab('smtp')}
                            className={`w-full text-left px-4 py-3 rounded-xl font-bold transition-all flex items-center gap-3 ${
                                configSubTab === 'smtp'
                                    ? 'bg-slate-900 text-white shadow-md'
                                    : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200'
                        ...
                        <button
                            onClick={() => setConfigSubTab('add_room')}
                            className={`w-full text-left px-4 py-3 rounded-xl font-bold transition-all flex items-center gap-3 ${
                                configSubTab === 'add_room'
                                    ? 'bg-slate-900 text-white shadow-md'
                                    : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200'
                            }`}
                        >
                        ...
                        <button
                            onClick={() => setConfigSubTab('tariff')}
                            className={`w-full text-left px-4 py-3 rounded-xl font-bold transition-all flex items-center gap-3 ${
                                configSubTab === 'tariff'
                                    ? 'bg-slate-900 text-white shadow-md'
                                    : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200'
                        ...
                        {/* Sidebar Footer */}
                        <div className="mt-6 bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-center select-none hidden md:block">
                    ...
                    <div className="flex-1 min-w-0 w-full">
                        {configSubTab === 'billing' && <InstitutionConfigForm mode="billing" />}
                        {configSubTab === 'policies' && <InstitutionConfigForm mode="policies" />}
                        {configSubTab === 'smtp' && <SMTPSettingsForm />}
                        {configSubTab === 'add_room' && <AddRoomForm />}
                        {configSubTab === 'tariff' && <TariffConfigForm />}
                    </div>
                </div>
            )}
