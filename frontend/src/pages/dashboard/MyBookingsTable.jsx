    ...
    return (
      <div className="text-center py-20 bg-slate-50 rounded-2xl border border-dashed border-slate-300">
        ...
        <p className="text-slate-500 max-w-md mx-auto">
          You have not submitted any accommodation requests yet.
        </p>
      </div>
    );
  }
        ...
        <tbody className="divide-y divide-slate-100">
          {bookings.map((booking) => (
            <tr key={booking.booking_id} className="hover:bg-slate-50 transition-colors">
              ...
              <td className="p-4 text-sm font-medium text-slate-800">
                {new Date(booking.departure_datetime).toLocaleDateString()}
              </td>
              ...
              <td className="p-4 text-right space-x-2 whitespace-nowrap">
                ...
                {booking.booking_state.startsWith('PENDING_') && (
                  <button
                    ...
                    className="inline-flex items-center p-1.5 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 rounded-lg transition-colors border border-red-200 shadow-sm ml-2"
